...
import { lineaAfecta, librosDe, tratamientoUniforme } from '@/shared/asiento/libro'
import { MarcaLibros } from '@/shared/asiento/Libros'
 ...
 * disposición para que quien revisa reconozca lo que verá después en el
 * libro, totales por libro incluidos: cada uno cuadra por su cuenta.
 */
    ...
    <div className="overflow-x-auto">
      ...
      <table className="w-full text-sm" aria-label="Asiento propuesto">
        ...
        <tbody>
          {asiento.lineas.map((linea, i) => (
            <tr key={i} className="border-b border-slate-100 last:border-0">
              ...
              <td className="px-3 py-1.5 text-slate-700">{nombreCuenta(linea.cuenta)}</td>
        ...
        <tfoot className="bg-slate-50 font-semibold text-slate-800">
          {filasTotales.map((t) => (
            <tr key={t.libro}>
              <td colSpan={uniforme ? 3 : 4} className="px-4 py-2 text-right text-xs">
                Totales
